...
int pgalloc(struct pcb_t *proc, uint32_t size, uint32_t reg_index)
{
  ...
  //*CHECKING ALLOC OUTPUT
#ifdef IODUMP

  #ifdef OUTPUT_FOLDER
    FILE *output_file = proc->file;
    fprintf(output_file, "===== AFTER TASK: ALLOCATION, INFO OF PHYSICAL MEMORY =====\n");
    fprintf(output_file, "pid of process = %d, region = %d, address =%08x, size =%d byte\n", proc->pid, reg_index, addr, size);
  #endif

  printf("===== AFTER TASK: DEALLOCATION, INFO OF PHYSICAL MEMORY =====\n");
  printf("pid of process = %d, region = %d, address =%08x, size =%d byte\n", proc->pid, reg_index, addr, size);
...
#ifdef OUTPUT_FOLDER
  FILE *output_file = proc->file;
  fprintf(output_file, "===== AFTER TASK: DEALLOCATION, INFO OF PHYSICAL MEMORY =====\n");
...
#ifdef OUTPUT_FOLDER
  FILE *output_file = proc->file;
  fprintf(output_file, "===== AFTER TASK: READING, INFO OF PHYSICAL MEMORY =====\n");
  fprintf(output_file, "read region=%d offset=%d value=%d\n", source, offset, data);
#endif

  printf("===== AFTER TASK: READING, INFO OF PHYSICAL MEMORY =====\n");
...
#ifdef OUTPUT_FOLDER
  FILE *output_file = proc->file;
  fprintf(output_file, "---- AFTER TASK: WRITING, PHYSICAL MEMORY INFO: ----\n");
  fprintf(output_file, "write region=%d offset=%d value=%d\n", destination, offset, data);
#endif

  printf("---- AFTER TASK: WRITING, PHYSICAL MEMORY INFO: ----\n");
